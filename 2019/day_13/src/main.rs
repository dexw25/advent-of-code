...
impl Game {
    pub fn new (width: usize, height: usize, prog: &Vec<i64>) -> Game {
        ...
        let score: i64 = 0;
        Game{frame, cpu, score, height, width}
    }
                ...
                true => {
                    ...
                    while self.cpu.output_available() > 3 {
                        ...
                        if x < 0 || y == 0 {
                            ...
                            if self.frame[(x as usize) + (y as usize) * self.width] == Tile::Ball && id == Tile::Empty {
                                defer_clear = true;
                                ball_pos.x = x;
                                ball_pos.y = y;
                            } else {
                    ...
                    println!("Score: {}\r\nEnter Input, A|a for left, D|d for right, S|s for stay", self.score);
                    if defer_clear {
                        self.frame[(ball_pos.x as usize) + (ball_pos.y as usize) * self.width] = Tile::Empty;
                        defer_clear = false;
                    }
